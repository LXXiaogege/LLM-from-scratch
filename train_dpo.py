...
from datasets.llm_dataset import DPODataset
...
def train_epoch(epoch, wandb):
    ...
    for step, batch in enumerate(train_loader):
        ...
        if (step + 1) % args.save_interval == 0 and (not ddp or dist.get_rank() == 0):
            ...
            ckp = f'{args.save_dir}/rlhf_{lm_config.embedding_dim}{moe_path}.pth'
...
def init_model(lm_config):
    ...
    moe_path = '_moe' if lm_config.use_moe else ''
    ckp = f'/Users/lvxin/datasets/llm/full_sft_{lm_config.embedding_dim}{moe_path}.pth'
    state_dict = torch.load(ckp, map_location=args.device)
    model.load_state_dict(state_dict, strict=False)
    # 初始化参考模型
    ref_model = LLMModel(lm_config)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="/Users/lvxin/datasets/llm/dpo.jsonl")
    ...
    # 初始化俩个SFT模型，一个policy model 和一个ref model，policy
    # policy model 用于训练，reference model 用于作为参考，防止policy model 偏离原SFT模型太远
